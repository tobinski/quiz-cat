...
function fca_qc_admin_cpt_script( $hook ) {
	...
	if ( ($hook == 'post-new.php' || $hook == 'post.php')  &&  $post->post_type === 'fca_qc_quiz' ) {  
		...
		$admin_data = array (
			...
			'questionDiv' => 	fca_qc_render_question_meta_box( array(), '{{QUESTION_NUMBER}}' ),
			'resultDiv' 	=> 	fca_qc_render_result_meta_box( array(), '{{RESULT_NUMBER}}' ),
...
function fca_qc_render_description_meta_box( $post ) {
	...
	echo "<input type='text' name='fca_qc_quiz_preview_url' id='fca_qc_quiz_preview_url'  hidden readonly data='" . get_permalink( $post->ID ) . "'>";
	...
	echo fca_qc_add_image_input( $quiz_meta['desc_img_src'], 'fca_qc_quiz_description_image_src', 'fca_qc_quiz_description_image_src' );
...
function fca_qc_render_questions_meta_box( $post ) {
	...
	if ( empty ( $questions ) ) {
		...
		echo fca_qc_render_question_meta_box( array(), 1 );
		...
		forEach ( $questions as $question ) {
			...
			echo fca_qc_render_question_meta_box( $question, $counter );
			$counter = $counter + 1;
...
// OUTPUT: HTML 
function fca_qc_render_question_meta_box( $question, $question_number ) {
	...
	$html = "<div class='fca_qc_question_item fca_qc_deletable_item' id='fca_qc_question_$question_number'>";
		...
		$html .= "<h3 class='fca_qc_question_label'><span class='fca_qc_quiz_heading_question_number'>" . __('Question', 'quiz-cat') . ' ' . $question_number . ": </span><span class='fca_qc_quiz_heading_text'>". fca_qc_convert_entities($question['question']) . "</span></h3>";
			...
			$html .= "<div class='fca_qc_question_input_div'>";
				...
				$html .= fca_qc_add_image_input( $question['img'], 'fca_qc_quiz_question_image_src[]' );				
...
function fca_qc_render_add_result_meta_box( $post ) {
	...
	if ( empty ( $results ) ) {
		...
		echo fca_qc_render_result_meta_box( array(), 1 );
		...
		forEach ( $results as $result ) {
			...
			echo fca_qc_render_result_meta_box( $result, $counter );
...
// OUTPUT: HTML
function fca_qc_render_result_meta_box( $result, $result_number ) {
	...
	$html = "<div class='fca_qc_result_item fca_qc_deletable_item' id='fca_qc_result_$result_number'>";
		...
		$html .= "<div class='fca_qc_result_input_div'>";
				...
				$html .= fca_qc_add_image_input( $result['img'], 'fca_qc_quiz_result_image_src[]' );
		...
		<div class='fca_qc_quiz' id='<?php echo "fca_qc_quiz_$post_id" ?>'>
			...
			<div class='flip-container fca_qc_quiz_div' style='display: none;'>
				<div class='fca-qc-flipper'>
					<?php echo fca_qc_do_question_panel() ?> 
					<?php echo fca_qc_do_answer_panel() ?> 
					
				</div>
			</div>
			<?php echo fca_qc_do_score_panel() ?> 
			...
			<?php echo fca_qc_do_result_panel() ?> 
...
function fca_qc_do_question_panel() {
	global $quiz_text_strings;
...
function fca_qc_do_answer_panel() {
	global $quiz_text_strings;
...
function fca_qc_do_score_panel() {
	global $quiz_text_strings;
...
function fca_qc_do_result_panel() {
	global $quiz_text_strings;
...
function fca_qc_add_image_input($img = '', $name = '', $id = '') {
	...
	$html .= "<button type='button' class='button-secondary fca_qc_quiz_image_upload_btn'>" . __('Add Image', 'quiz-cat') . "</button>";
	...
	$html .= "<div class='fca_qc_image_hover_controls'>";
		...
		if ( empty ( $img ) ) {
			$html .= "<button type='button' class='button-secondary fca_qc_quiz_image_change_btn' style='display:none;'>" . __('Change', 'quiz-cat') . "</button>";
